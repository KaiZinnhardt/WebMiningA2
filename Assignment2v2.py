...
#To visualize the tree plot
import squarify
#To visualize to word of clouds
from wordcloud import WordCloud

#libraries for displaying playlist Image
...
#get audio features
def get_genre(artist_ids):
  """
  The function has one input parameter. The artist_ids contain the ids of the artists of each track in a playlist.
  :rtype: list
  """
  genres=[]
  ...
  for i in range(0, len(artist_ids), 50):
    ...
    #append the genres into the corresponding array
...
def extract_artist_ids(playlist):
    """
    The function extracts the ids from the artists of each playlist item from the playlist. After extracting the names it is stored in a list.
    :rtype: list
    """
    artist_ids = []
    
    # Navigate through the JSON structure to access each artist id
    for item in playlist['tracks']['items']:
...
# get popularity
def extract_popularity_score(playlist):
    """
...
# get years
def extract_release_years(playlist):
    """
...
def generate_cloud_of_words(ids1, ids2):
    """
    Generates the world cloud objects from two URLs corresponding to the playlists.
    :rtype: tuple
    """
    #Get the artist ids in the playlist


    #Get the genres for each  playlist
    genres_list1 = get_genre(ids1)
    genres_list2 = get_genre(ids2)

    #modify genres
    modified_genres1 = [s.replace(" ", "-") for s in genres_list1]
    modified_genres2 = [s.replace(" ", "-") for s in genres_list2]

    #Extract the genres
    text_1 = ' '.join(modified_genres1)
    text_2 = ' '.join(modified_genres2)

    # Generate word cloud
    wordcloud_1 = WordCloud(width=800, height=400, background_color='white',regexp=r"\w[\w'-]*\w").generate(text_1)
    wordcloud_2 = WordCloud(width=800, height=400, background_color='white',regexp=r"\w[\w'-]*\w").generate(text_2)
...
def main():
    ...
    if st.button("Compare Playlists"):
        ...
        try:
            ...
            # Retrieve the Playlist item IDS, artist_ids and playlist name
            songs_list_1 = get_songs_id_playlist(playlist_1)
            songs_list_2 = get_songs_id_playlist(playlist_2)
            artist_ids_1 = extract_artist_ids(playlist_1)
            artist_ids_2 = extract_artist_ids(playlist_2)
            ...
            with col1:
                st.markdown("### *"+play_list_name_1+" ("+str(len(songs_list_1))+" Songs)*")  # Bigger and bolder title
                st.image(image1)

            with col2:
                st.markdown("### *"+play_list_name_2+" ("+str(len(songs_list_2))+" Songs)*")  # Bigger and bolder title
                st.image(image2)
            ...
            with placeholder.container():
                with st.spinner('Building Wordcloud'):
                    try:
                        result = generate_cloud_of_words(artist_ids_1,artist_ids_2)
                        print("This is the result:")
                        #print(result)
                        cloud_of_words(result[0],result[1],play_list_name_1,play_list_name_2)
            ...
            if e.http_status == 404:
                st.markdown("404: Playlist not found.")
